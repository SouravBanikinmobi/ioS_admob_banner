...
- (void)requestBannerAd:(GADAdSize)adSize
  ...
  @try {
      ...
      self.banner.keyWords = request.userKeywords;
      [self.banner loadAd];
  }
  @catch (NSException *e) {
      [self.delegate customEventBanner:self didFailAd:[AerServExceptions InvalidRequest]];
  }
}
...
- (void)adViewDidFailToLoadAd:(ASAdView *)adView withError:(NSError *)error {
    [adView stopAutomaticallyRefreshingContents];
    [self.delegate customEventBanner:self didFailAd:[AerServExceptions NoFill]];
}
